...
from sef_gram.world_rollout_envs import GridWorldRolloutEnv, LinePhysicsRolloutEnv, rollout_envs


def test_rollout_envs_step_shapes():
    torch.manual_seed(0)
    device = torch.device("cpu")
    for env in (GridWorldRolloutEnv(max_obs_dim=16), LinePhysicsRolloutEnv(max_obs_dim=16)):
    ...
    assert set(rollout_envs(max_obs_dim=16)) == {"gridworld", "line_physics"}
...
def test_true_rollout_eval_smoke():
    ...
    for row in rows:
        ...
        assert row["env"] in {"gridworld", "line_physics"}
